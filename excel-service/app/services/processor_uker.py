...
def process_uker_from_df(
    ...
    segmen_col = _find_col(df_p_all, "SEGMEN_2025", "Segmen_2025", "Segmen", "SEGMEN", "Segmentasi BPR")
    ...
    if 'Segmen' in df_p_all.columns and 'SEGMEN_2025' in df_p_all.columns:
        mask_nan = df_p_all['SEGMEN_2025'].isna() | (df_p_all['SEGMEN_2025'].astype(str).str.strip() == '')
        df_p_all.loc[mask_nan, 'SEGMEN_2025'] = df_p_all.loc[mask_nan, 'Segmen']
        print(f"[UKER SEGMEN FALLBACK] {mask_nan.sum()} baris diisi dari kolom 'Segmen'")
    ...
    max_s = df_s_all['_tanggal'].max() if '_tanggal' in df_s_all.columns and not df_s_all['_tanggal'].dropna().empty else None
    max_p = df_p_all['_tanggal'].max() if '_tanggal' in df_p_all.columns and not df_p_all['_tanggal'].dropna().empty else None
    
    all_dates = [d for d in (max_s, max_p) if pd.notnull(d)]
    global_max_tgl = max(all_dates) if all_dates else None

    def _apply_lbl_uker(ts):
        if pd.isnull(ts):
            return "Unknown"
        is_latest = False
