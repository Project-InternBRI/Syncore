...
def get_default_export_filename(data: dict, kc_name: str = "AH Gunsar") -> str:
    ...
    try:
        ...
        if meta_row:
            ...
            from app.services.file_reader import BULAN_PANJANG
            if terbaru_dt:
...
CLR_GROWTH_GREEN  = "16A34A"   # Hijau — surplus / kondisi baik
CLR_GROWTH_RED    = "DC2626"   # Merah — defisit / kondisi buruk
CLR_INFO_BG       = "1E3A5F"
CLR_INFO2_BG      = "EFF6FF"

def _growth_font(num_val: float, is_terbalik: bool, is_bold: bool = False) -> Font:
    """
    Tentukan font warna untuk cell growth.
    - Positif + baris normal     → hijau  (surplus)
    - Positif + baris terbalik   → merah  (SML/NPL naik = buruk)
    - Negatif + baris normal     → merah  (defisit)
    - Negatif + baris terbalik   → hijau  (SML/NPL turun = baik)
    - Nol                        → abu-abu
    """
    if num_val == 0:
        return _font(bold=is_bold, color=CLR_ZERO_FONT)
    if num_val > 0:
        clr = CLR_GROWTH_RED if is_terbalik else CLR_GROWTH_GREEN
    else:
        clr = CLR_GROWTH_GREEN if is_terbalik else CLR_GROWTH_RED
    return _font(bold=is_bold, color=clr)
...
def export_to_excel(data_dict: dict,
    ...
    for rka in rka_payload:
        ...
        kc_short = kc_full.replace('KC Jakarta ', '').replace('KC ', '')
    ...
    for kc_name in kc_order:
        if kc_name in data_dict:
            ...
            # Pass RKA for this specific branch
            rka_for_branch = rka_records_by_month_and_kc.get(kc_name, {})
            _write_sheet(ws, kc_name, kc_data, rka_for_branch)
...
# TULIS SATU SHEET
# ────────────────────────────────────────────────────────────────────
def _write_sheet(ws, kc_name: str, kc_data: dict, rka_records_by_month: dict = None) -> None:
    if rka_records_by_month is None:
    ...
    # === FETCH RKA DATA ===
    # RKA data is now passed into _write_sheet via rka_records_by_month argument.
...
def _write_header_value(ws, row: int, row_data: dict, rka_vals: list,
    ...
    from app.services.processor import hitung_pencapaian_rka
    val_terbaru = None
    if periode_list:
