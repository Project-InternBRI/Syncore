...
def format_label(ts: pd.Timestamp, is_latest_month: bool = False) -> str:
    """
    ...
    # Jangan gabungkan menjadi "Bulan-Tahun" jika ini adalah bulan berjalan (latest month)
    # karena kita ingin melihat data per harinya.
    if ts.day >= akhir - 1 and not is_latest_month:
        return f"{bln}-{thn2}"
    else:
...
def hitung_growth(nama_baris, nilai_terbaru, nilai_ref):
    ...
    # Selalu kembalikan selisih mentah (current - reference)
    # Pewarnaan terbalik (SML/NPL: turun=hijau, naik=merah) ditangani di exporter
    return selisih
...
def hitung_semua_growth_dan_rka(data_per_periode, rka_values,
    ...
    terbaru_dt = periode_refs['terbaru']
    def _lbl(ts):
        if not ts: return None
        is_latest = (ts.month == terbaru_dt.month and ts.year == terbaru_dt.year)
        return format_label(ts, is_latest_month=is_latest)

    terbaru_label  = _lbl(periode_refs['terbaru'])
    dtd_label      = _lbl(periode_refs['dtd_ref'])
    mtd_label      = _lbl(periode_refs['mtd_ref'])
    yoy_label      = _lbl(periode_refs['yoy_ref'])
    ytd_label      = _lbl(periode_refs['ytd_ref'])
...
def _attach_growth(rows: list[dict], periodes_sorted: list[tuple]) -> list[dict]:
    ...
    for r in rows:
        ...
        # Tandai baris yang warna growth-nya terbalik (SML/NPL: turun=hijau)
        r['is_terbalik'] = nama_baris in TIPE_GROWTH_TERBALIK
...
def _growth_labels(periodes_sorted: list[tuple]) -> dict:
    ...
    try:
        from app.services.file_reader import BULAN_SINGKAT
    except ImportError:
    ...
    if terbaru_dt:
        ...
        yoy_ref = refs['yoy_ref']
        if yoy_ref:
            y_str = f"{BULAN_SINGKAT[yoy_ref.month]} {str(yoy_ref.year)[-2:]}"
            labels['yoy_label'] = f"YOY\n({y_str} vs {t_str})"
            
        ytd_ref = refs['ytd_ref']
        if ytd_ref:
            ytd_str = f"{BULAN_SINGKAT[ytd_ref.month]} {str(ytd_ref.year)[-2:]}"
            labels['ytd_label'] = f"YTD\n({ytd_str} vs {t_str})"
    return labels
...
def process_files(
    ...
    # Kita cari max_tanggal dari seluruh data untuk menentukan "latest month"
    max_s = df_s['_tanggal'].max() if '_tanggal' in df_s.columns and not df_s['_tanggal'].dropna().empty else None
    max_p = df_p['_tanggal'].max() if '_tanggal' in df_p.columns and not df_p['_tanggal'].dropna().empty else None
    
    all_dates = [d for d in (max_s, max_p) if pd.notnull(d)]
    global_max_tgl = max(all_dates) if all_dates else None

    def _apply_lbl(ts):
        if pd.isnull(ts):
            return "Unknown"
        is_latest = False
        if global_max_tgl is not None:
            if ts.month == global_max_tgl.month and ts.year == global_max_tgl.year:
                is_latest = True
        return format_label(ts, is_latest_month=is_latest)
    ...
    try:
        from app.services.processor_uker import process_uker_from_df
