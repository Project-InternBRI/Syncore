...
def export_monitoring_produk_to_excel(data_dict: dict, output_path: str):
    ...
    terbaru_dt = None
    if meta:
        val = meta['periode_refs']['terbaru']
        if isinstance(val, str):
            try:
                # Handle isoformat or other formats
                terbaru_dt = pd.to_datetime(val).to_pydatetime()
            except:
                terbaru_dt = None
        else:
            terbaru_dt = val
    ...
    for title, row_label, row_type in tables_config:
        ...
        for kc in kc_names:
            ...
            if target_r:
                ...
                terbaru_label = format_label(terbaru_dt, is_latest_month=True) if terbaru_dt else periode_list[-1]
