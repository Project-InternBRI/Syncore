...
new_func = """def export_pdf_visual(data_dict, output_path, metadata):
    ...
    # --- Cover Page Styles ---
    cover_title_style = ParagraphStyle('CoverTitle', parent=styles['Heading1'], fontSize=48, textColor=colors.white, alignment=0, spaceBefore=140, spaceAfter=30, leftIndent=40)
    ...
    cover_bottom_style = ParagraphStyle('CoverBottom', parent=styles['Normal'], fontSize=18, textColor=colors.white, alignment=0, spaceBefore=120, leftIndent=40, leading=22)
    ...
    if sml is not None: kpi_headers.append("SML Ratio (%)"); kpi_values.append(f"{sml:.2f}%")
    if npl is not None: kpi_headers.append("NPL Ratio (%)"); kpi_values.append(f"{npl:.2f}%")
    ...
    for kc_nm in kcs:
        ...
        for row in kc_r:
            ...
            for m in [row.get("mtd"), row.get("ytd"), row.get("yoy")]:
                ...
                else: r_data.append(f"{m:.2f}%" if "%" in lbl else f"{m:,.0f}")
